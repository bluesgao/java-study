...
//                System.out.println("-------------------------------------------");
    ...
    public void checkAndPut(String tableName,String rowKey, String familyName, String condition, String expect, String[] columns, String[] values){
        // 获取表
        Table table= null;
        try {
            table=getTable(tableName);
            ...
            table.checkAndMutate(Bytes.toBytes(rowKey),Bytes.toBytes(familyName))
                    .qualifier(Bytes.toBytes(condition))
                    .ifMatches(CompareOperator.GREATER_OR_EQUAL,Bytes.toBytes(expect))
                    .thenPut(put);
            log.debug("checkAndPut add or update data Success,rowKey:" + rowKey);
            table.close();
        } catch (Exception e) {
            log.error(MessageFormat.format("checkAndPut 为表添加 or 更新数据失败,tableName:{0},rowKey:{1},familyName:{2}"
                    ,tableName,rowKey,familyName),e);
        }finally {
            close(null,null,table);
        }
